                ...
                <div className="text-center">
                    ...
                    {!authenticated ? (
                        ...
                        <div className="space-y-4">
                            ...
                            <Link href="/hive" className="inline-block text-zinc-400 hover:text-white underline underline-offset-4 text-sm font-mono uppercase">
  ...
  return (
    <div className="min-h-screen bg-[#020202] text-white pt-24 pb-12 px-4 md:px-8 max-w-7xl mx-auto font-sans selection:bg-violet-600 selection:text-white">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-12 gap-6">
        <div>
            <h1 className="text-3xl font-bold font-mono uppercase tracking-tighter mb-2">Validation Dashboard</h1>
        ...
        <div className="flex items-center gap-4">
             <button 
                ...
                className="p-2 bg-zinc-900 border border-zinc-800 rounded-sm hover:bg-zinc-800 transition-colors"
                title="Refresh Submissions"
             >
                 <RefreshCw size={16} className={`text-zinc-400 ${loadingSubmissions ? "animate-spin" : ""}`} />
             </button>
            <div className="bg-zinc-900 px-4 py-2 rounded-sm border border-zinc-800 text-xs font-mono text-zinc-400 flex items-center gap-2">
        ...
        <div className="divide-y divide-zinc-800">
            {loadingSubmissions ? (
                ...
                <div className="p-12 text-center text-zinc-500">
                    <p>No submissions found within recent blocks.</p>
                </div>
            ) : (
                submissions.map((sub) => {
                        ...
                        <div key={`${sub.id}-${sub.transactionHash}`} className="p-6 hover:bg-white/5 transition-colors flex flex-col md:flex-row md:items-center justify-between gap-6 group">
                            <div className="space-y-3">
                                <div className="flex items-center gap-3">
                                    <span className="text-xl font-bold font-mono text-white">BOUNTY #{sub.id}</span>
                                    <span className="text-zinc-600 text-xs font-mono border border-zinc-800 px-2 py-0.5 rounded-sm">
                                        {new Date(sub.timestamp).toLocaleTimeString()}
                                    </span>
                                </div>
                                <div className="flex flex-col gap-1 text-sm font-mono text-zinc-400">
                                     <div>AGENT: <span className="text-blue-400">{sub.agentAddress}</span></div>
                                     <div className="truncate max-w-md text-zinc-600 text-xs">HASH: {sub.reportUri}</div>
                                </div>
      ...
      {isSuccess && (
          <div className="mt-8 p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-sm flex items-center gap-3 text-emerald-400 font-mono text-sm animate-in slide-in-from-bottom-2">
              <Check className="w-5 h-5" />
              <span>BOUNTY FINALIZED: FUNDS RELEASED & REPUTATION UPDATED.</span>
          </div>
      )}
    </div>
  );
